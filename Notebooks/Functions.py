...
#Builds an OLS model
def build_sm_ols(df, features, target, add_constant=False, show_summary=True):
    X = df[features]
    if add_constant:
        X = sm.add_constant(X)
    y = df[target]
...
#Checks for normality in the residuals of the OLS model using the Shapiro test with an p value < 0.05
...
def check_vif(df, features_to_use, target_feature):
    ols = build_sm_ols(df=df, features_to_use=features, target=target_feature, show_summary=False)
    r2 = ols.rsquared
    return 1 / (1 - r2)
...
def check_vif_feature_space(df, features, vif_threshold=3.0):
    all_good_vif = True
    for feature in features:
        target_feature = feature
        _features_to_use = [f for f in features if f!=target_feature]
...
def check_model(df, 
    ...
    # build model 
    ols = build_sm_ols(df=df, features_to_use=features, 
